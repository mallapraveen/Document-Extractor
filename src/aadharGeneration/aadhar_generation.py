...
def print_front_layout(img: Image, data: dict, fonts: dict, dest_lang: str):
    ...
    image_editable.text((183, 1239), data['dob_lang'] + ' / ' + 'DOB:' + data['DOB'], (0, 0, 0), font=fonts['fnt'])
    image_editable.text((193, 1425), str(data['Aadhar_No']), (0, 0, 0), font=fonts['fnt2'])
    image_editable.text((180, 990), str(data['Aadhar_No']), (0, 0, 0), font=fonts['fnt2'])

    image_editable.text((255, 1470), str(data['Aadhar_No']), (0, 0, 0), font=fonts['vid'])
    image_editable.text((245, 1031), str(data['Aadhar_No']), (0, 0, 0), font=fonts['vid2'])

    if data['Gender'] == 'Male':
        image_editable.text((183, 1262), data['gender_male_lang'] + ' / ' + data['Gender'], (0, 0, 0),
                            font=fonts['fnt'])
    else:
        image_editable.text((183, 1262), data['gender_female_lang'] + ' / ' + data['Gender'], (0, 0, 0),
                            font=fonts['fnt'])
    ...
    image_editable.text((112, 445), 'S/O ' + data['Father_Name'], (0, 0, 0), font=fonts['fnt'])
    ...
    if data['Address2'] is not np.nan and data['Address3'] is not np.nan:
        image_editable.text((112, 495), str(data['Address2']), (0, 0, 0), font=fonts['fnt'])
    elif data['Address2'] is not np.nan:
        image_editable.text((112, 495), str(data['Address2']), (0, 0, 0), font=fonts['fnt'])

    if data['Address3'] is not np.nan and data['Address4'] is not np.nan:
        image_editable.text((112, 520), str(data['Address3']), (0, 0, 0), font=fonts['fnt'])
    elif data['Address3'] is not np.nan:
        image_editable.text((112, 520), str(data['Address3']), (0, 0, 0), font=fonts['fnt'])

    if data['Address4'] is not np.nan and data['Address5'] is not np.nan:
        image_editable.text((112, 545), str(data['Address4']), (0, 0, 0), font=fonts['fnt'])
    elif data['Address4'] is not np.nan:
        image_editable.text((112, 545), str(data['Address4']), (0, 0, 0), font=fonts['fnt'])

    if data['Address5'] is not np.nan:
        image_editable.text((112, 570), str(data['Address5']), (0, 0, 0), font=fonts['fnt'])
...
def print_back_layout(img: Image, data: dict, fonts: dict):
    ...
    image_editable.text((200, 1431), str(data['Aadhar_No']), (0, 0, 0), font=fonts['fnt2'])
    image_editable.text((260, 1468), str(data['Aadhar_No']), (0, 0, 0), font=fonts['vid2'])

    image_editable.text((29, 1258), 'Address: S/O ' + data['Father_Name'] + ",", (0, 0, 0), font=fonts['fnt'])
    image_editable.text((29, 1278), str(data['Address1']) + ', ', (0, 0, 0), font=fonts['fnt'])

    if data['Address2'] is not np.nan and data['Address3'] is not np.nan:
        image_editable.text((29, 1298), str(data['Address2']) + ', ', (0, 0, 0), font=fonts['fnt'])
    elif data['Address2'] is not np.nan:
        image_editable.text((29, 1298), str(data['Address2']), (0, 0, 0), font=fonts['fnt'])

    if data['Address3'] is not np.nan and data['Address4'] is not np.nan:
        image_editable.text((29, 1318), str(data['Address3']) + ', ', (0, 0, 0), font=fonts['fnt'])
    elif data['Address3'] is not np.nan:
        image_editable.text((29, 1318), str(data['Address3']), (0, 0, 0), font=fonts['fnt'])

    if data['Address4'] is not np.nan and data['Address5'] is not np.nan:
        image_editable.text((29, 1338), str(data['Address4']) + ', ', (0, 0, 0), font=fonts['fnt'])
    elif data['Address4'] is not np.nan:
        image_editable.text((29, 1338), str(data['Address4']), (0, 0, 0), font=fonts['fnt'])

    if data['Address5'] is not np.nan:
        image_editable.text((29, 1358), str(data['Address5']), (0, 0, 0), font=fonts['fnt'])
...
def create_small_aadhar(no_of_images: int, template_path: str, save_path: str):
    ...
    for i in tqdm.tqdm(range(no_of_images), total=no_of_images, unit='Image'):
        ...
        image_editable.text((99, 50), convert_to_lang(aadhar_data['Name'], src='en', dest=dest_lang), (0, 0, 0),
                            font=fnt)
        image_editable.text((99, 65), aadhar_data['Name'], (0, 0, 0), font=fnt)
        image_editable.text((99, 80), dob_lang + ' /' + ' DOB:' + aadhar_data['DOB'], (0, 0, 0), font=fnt)
        image_editable.text((99, 158), str(aadhar_data['Aadhar_No']), (0, 0, 0), font=fnt2)

        if aadhar_data['Gender'] == 'Male':
            image_editable.text((99, 95), gender_male_lang + ' / ' + aadhar_data['Gender'], (0, 0, 0), font=fnt)
        else:
            image_editable.text((99, 95), gender_female_lang + ' / ' + aadhar_data['Gender'], (0, 0, 0), font=fnt)

        image_editable = ImageDraw.Draw(img2)
        image_editable.text((99, 158), str(aadhar_data['Aadhar_No']), (0, 0, 0), font=fnt2)
        image_editable.text((26, 54), 'Address: S/O ' + aadhar_data['Father_Name'] + ",", (0, 0, 0), font=fnt3)

        image_editable.text((26, 66), aadhar_data['Address1'] + ', ', (0, 0, 0), font=fnt3)
        # image_editable.text((26, 85),  str(aadhar_data['Address2']), (0, 0, 0), font=fnt3)

        if aadhar_data['Address2'] is not np.nan and aadhar_data['Address3'] is not np.nan:
            image_editable.text((26, 78), str(aadhar_data['Address2']) + ', ', (0, 0, 0), font=fnt3)
        elif aadhar_data['Address2'] is not np.nan:
            image_editable.text((26, 78), str(aadhar_data['Address2']), (0, 0, 0), font=fnt3)

        if aadhar_data['Address3'] is not np.nan and aadhar_data['Address4'] is not np.nan:
            image_editable.text((26, 90), str(aadhar_data['Address3']) + ', ', (0, 0, 0), font=fnt3)
        elif aadhar_data['Address3'] is not np.nan:
            image_editable.text((26, 90), str(aadhar_data['Address3']), (0, 0, 0), font=fnt3)

        if aadhar_data['Address4'] is not np.nan and aadhar_data['Address5'] is not np.nan:
            image_editable.text((26, 102), str(aadhar_data['Address4']) + ', ', (0, 0, 0), font=fnt3)
        elif aadhar_data['Address4'] is not np.nan:
            image_editable.text((26, 102), str(aadhar_data['Address4']), (0, 0, 0), font=fnt3)

        if aadhar_data['Address5'] is not np.nan:
            image_editable.text((26, 114), str(aadhar_data['Address5']), (0, 0, 0), font=fnt3)
